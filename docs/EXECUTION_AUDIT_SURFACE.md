...
| `src/coordinator/coordinator.ts` — `DefaultToolCallStreamCoordinator` | Accumulates raw arguments, owns call identity/state, detects ordering/identity/SDK execution violations, finishes parsers, and resolves/runs registered `ToolInputValidator`s (0.5.0+: no longer compiles Ajv directly - see next row). | No; produces evidence and parser-level `executable`. | `test/unit/coordinator*.test.ts`, `test/unit/validator-independence.test.ts`, `test/integration/authority-boundaries.test.ts` |
| `src/validation/types.ts`, `src/validation/ajv-validator.ts`, `src/validation/standard-schema-validator.ts` (0.5.0+) | Builds the coordinator's validator map from the two explicit `toolSchemas`/`validators` options (`buildValidatorMap`) - a `toolSchemas` entry is always compiled through Ajv (statically imported, see `docs/RUNTIME_DEPENDENCIES.md`), a `validators` entry is always used as-is; a tool name in both is a construction-time error. No structural discrimination anywhere in this path. | No; produces the same `valid`/`errors` evidence Ajv previously produced directly. | `test/unit/validator-independence.test.ts`, `test/unit/coordinator-schema-validation.test.ts` |
