...
| `src/validation/types.ts`, `src/validation/ajv-validator.ts`, `src/validation/standard-schema-validator.ts` (0.5.0+) | Normalizes a registered `toolSchemas` entry (a `ToolInputValidator`, or a raw JSON Schema compiled lazily through the isolated Ajv adapter) into the `{validate}` shape the coordinator calls. | No; produces the same `valid`/`errors` evidence Ajv previously produced directly. | `test/unit/validator-independence.test.ts`, `test/unit/coordinator-schema-validation.test.ts` |
