...
| --- | --- | --- | --- | --- | --- |
| `ajv` | `8.20.0` | MIT | not declared | Direct dependency imported statically at the top of `src/validation/ajv-validator.ts` (0.5.0+), which the `toolSchemas`/`schemas` option (optional per-tool JSON Schema validators registered as a raw schema object, see `docs/VALIDATION.md`) compiles through. An earlier 0.5.0 draft deferred this import via a synchronous `require` (`node:module`'s `createRequire`); that was reverted after principal review because `ajv` is a hard dependency of this package either way (see "Not zero-runtime-dependency yet" in `docs/VALIDATION.md`), so deferring *when* it loads bought no real reduction in what a consumer installs while adding real ESM/bundler/edge portability risk of its own. The separate `validators` registration option (`ToolInputValidator`) has no dependency on this module or on `ajv` at all, conceptually or in its own import graph - but `ajv` itself remains present on disk for every consumer either way, exactly as before 0.5.0. An `Ajv` instance is created and a schema compiled synchronously and eagerly at registration time, matching the coordinator's own fail-fast-at-construction guarantee. | no `preinstall`, `install`, `postinstall`, or `prepare`; published manifest has `prepublish`, which is not observed during registry dependency installation |
