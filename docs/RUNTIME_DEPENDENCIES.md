...
| --- | --- | --- | --- | --- | --- |
| `ajv` | `8.20.0` | MIT | not declared | Direct dependency compiled through the isolated adapter in `src/validation/ajv-validator.ts` (0.5.0+) to serve optional per-tool JSON Schema validators registered as a raw schema object (see `docs/VALIDATION.md`). As of 0.5.0 it is **not** imported at the package entrypoint or at any module's top level — it is loaded lazily via a synchronous `require` (`node:module`'s `createRequire`) only from inside that one adapter function, and only the first time a raw JSON Schema value is actually registered; a caller who only ever registers a `ToolInputValidator` never causes it to load at all. An `Ajv` instance is still created and a schema still compiled synchronously and eagerly at that point, matching the coordinator's own fail-fast-at-construction guarantee. | no `preinstall`, `install`, `postinstall`, or `prepare`; published manifest has `prepublish`, which is not observed during registry dependency installation |
