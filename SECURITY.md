...
- **Runtime dependencies**: validation is validator-agnostic (see `docs/VALIDATION.md`); a raw JSON Schema value is still compiled via a lazily-loaded AJV. Dependency changes should be reviewed with the same safety and maintenance scrutiny as parser changes.
